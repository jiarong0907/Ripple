...
control filter_link_classification1_2(inout headers hdr,
                  inout metadata meta,
                  inout standard_metadata_t standard_metadata) {
...
control reduce_pktlen_to_inflowsz_update(inout headers hdr,
    ...
    apply{
        ...
        {NUM_RAND1, meta.dip, NUM_RAND2}, (bit<16>)REDUCE_PKTLEN_TO_INFLOWSZ_SIZE);
        hash(reduce_pktlen_to_inflowsz_idx1, HashAlgorithm.crc16, (bit<32>)0,
        {NUM_RAND1, NUM_RAND2, meta.dip}, (bit<16>)REDUCE_PKTLEN_TO_INFLOWSZ_SIZE);
        hash(reduce_pktlen_to_inflowsz_idx2, HashAlgorithm.crc16, (bit<32>)0,
        {meta.dip, NUM_RAND1, NUM_RAND2}, (bit<16>)REDUCE_PKTLEN_TO_INFLOWSZ_SIZE);
...
control filter_victimLks_size_classification2_5(inout headers hdr,
                  inout metadata meta,
                  inout standard_metadata_t standard_metadata,
                  in register<bit<32>> victimLks_size) {
...
control filter_link_classification2_6(inout headers hdr,
                  inout metadata meta,
                  inout standard_metadata_t standard_metadata) {
...
control reduce_pktlen_to_egflowsz_update(inout headers hdr,
    ...
    apply{
        ...
        {NUM_RAND1, meta.dip, NUM_RAND2}, (bit<16>)REDUCE_PKTLEN_TO_EGFLOWSZ_SIZE);
        hash(reduce_pktlen_to_egflowsz_idx1, HashAlgorithm.crc16, (bit<32>)0,
        {NUM_RAND1, NUM_RAND2, meta.dip}, (bit<16>)REDUCE_PKTLEN_TO_EGFLOWSZ_SIZE);
        hash(reduce_pktlen_to_egflowsz_idx2, HashAlgorithm.crc16, (bit<32>)0,
        {meta.dip, NUM_RAND1, NUM_RAND2}, (bit<16>)REDUCE_PKTLEN_TO_EGFLOWSZ_SIZE);
...
control filter_victimLks_size_classification3_9(inout headers hdr,
                  inout metadata meta,
                  inout standard_metadata_t standard_metadata,
                  in register<bit<32>> victimLks_size) {
...
control filter_link_classification3_10(inout headers hdr,
                  inout metadata meta,
                  inout standard_metadata_t standard_metadata) {
...
control filter_inflowsz_egflowsz_classification3_12(inout headers hdr,
                  inout metadata meta,
                  inout standard_metadata_t standard_metadata,
                  in register<bit<32>> reduce_pktlen_to_inflowsz_cur_wind_pass,
                  in register<bit<32>> reduce_pktlen_to_egflowsz_cur_wind_pass) {
...
control filter_inflowsz_egflowsz_return(inout headers hdr,
                  inout metadata meta,
                  inout standard_metadata_t standard_metadata,
                  in register<bit<32>> victim) {
    ...
    apply{
        ...
        hash(victim_idx0, HashAlgorithm.crc16, (bit<32>)0,
        {NUM_RAND1, meta.dip, NUM_RAND2}, (bit<16>)VICTIM_SIZE);
        hash(victim_idx1, HashAlgorithm.crc16, (bit<32>)0,
        {NUM_RAND1, NUM_RAND2, meta.dip}, (bit<16>)VICTIM_SIZE);
        hash(victim_idx2, HashAlgorithm.crc16, (bit<32>)0,
        {meta.dip, NUM_RAND1, NUM_RAND2}, (bit<16>)VICTIM_SIZE);

        if (hdr.ipv4.protocol == TCP_PROTOCOL){
            victim.write(victim_idx0, 1);
            victim.write(victim_idx1, 1);
            victim.write(victim_idx2, 1);
        }
    }
}
...
control filter_dip_in_victim_mitigation1(inout headers hdr,
                  inout metadata meta,
                  inout standard_metadata_t standard_metadata,
                  in register<bit<32>> victim) {
    ...
    apply{
        ...
        victim.read(victim_val0, victim_idx0);
        victim.read(victim_val1, victim_idx1);
        victim.read(victim_val2, victim_idx2);

        if (victim_val0 == 1 && victim_val1 == 1 && victim_val2 == 1) {
            meta.mitigation_filter1 = 1;
        } else {
...
control ctrl_sync(inout headers hdr,
    ...
    apply{
        ...
        DEF_VAR_64(victim_val)
        READ_REG_64(victim, victim_val)
        ...
        if (meta.swid == 3) { // The root
            if (hdr.sync.srcSWUID == 3) { // downstream packets
                ...
                SET_HDR_64_OR(hdr.sync.victim, victim_val)
            }
            // upstream packets
            else {
                ...
                UPDATE_REG_BF_64(victim, victim_val, hdr.sync.victim)
            }
        }
        // not root
        else {
