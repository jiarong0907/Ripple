...
const bit<32> KEYFLOWS_SIZE = 65535;
...
control MyIngress(inout headers hdr,
    ...
    register <bit<32>>(65536) keyflows;
...
*************************************************************************/
		...
		if (meta.classification_filter1 == 1) {
                filter_link_classification1_1.apply(hdr, meta, standard_metadata);
		...
		if (meta.classification_filter2 == 1) {
                filter_link_classification2_4.apply(hdr, meta, standard_metadata);
		...
		if (meta.classification_filter3 == 1) {
                filter_link_classification3_7.apply(hdr, meta, standard_metadata);
		...
		if (meta.classification_filter3 == 1) {
                filter_inflowsz_egflowsz_classification3_9.apply(hdr, meta, standard_metadata, \
                            reduce_pktlen_to_inflowsz_cur_wind_pass, \
                            reduce_pktlen_to_egflowsz_cur_wind_pass);

	 	}
		if (meta.classification_filter3 == 1) {
                filter_ipset_sip_classification3.apply(hdr, meta, standard_metadata);

                if (meta.classification_filter3 == 1){
                    filter_sip_return.apply(hdr, meta, standard_metadata, \
                                keyflows);
                }
	 	}
...
*******************************************************************************/
		...
		if (meta.mitigation_filter1 == 1) {
                filter_dip_in_keyflows_mitigation1.apply(hdr, meta, standard_metadata, \
                            keyflows);
