...
control filter_victimLks_size_classification11_1(inout headers hdr,
                  inout metadata meta,
                  inout standard_metadata_t standard_metadata,
                  in register<bit<32>> victimLks_size) {
    ...
    apply{
        ...
        if (meta.victimLks_size > 1) {
            ...
            meta.classification1_filter1 = 0; //no high load, clean this flag
        }
    }
}

control filter_link_in_victimLks_classification11(inout headers hdr,
                  inout metadata meta,
                  inout standard_metadata_t standard_metadata,
                  in register<bit<32>> victimLks) {
...
control reduce_pktlen_to_flowsz_window(inout headers hdr,
                  ...
                  in register<bit<48>> c_wind_last_ts_p1) {
    ...
    apply{
        ...
        if (meta.reduce_pktlen_to_flowsz_wind_updated == 0 && meta.c_wind_interval_p1 > 100 * 1000){
            meta.reduce_pktlen_to_flowsz_wind_updated = 1;
            c_wind_last_ts_p1.write(0, meta.this_ts_val);
...
control filter_flowsz_classification11_4(inout headers hdr,
                  inout metadata meta,
                  inout standard_metadata_t standard_metadata,
                  in register<bit<32>> reduce_pktlen_to_flowsz_cur_wind_pass) {
    ...
    apply{
        ...
        if (meta.reduce_pktlen_to_flowsz_full > 11000 && reduce_pktlen_to_flowsz_cur_wind_pass_val >= 1) {
            meta.classification1_filter1 = 1;
        } else {
            meta.classification1_filter1 = 0; //no high load, clean this flag
        }
...
control filter_flowsz_return(inout headers hdr,
                  inout metadata meta,
                  inout standard_metadata_t standard_metadata,
                  in register<bit<32>> mal_coremelt) {

    apply{

        bit<32> mal_coremelt_idx0;
        bit<32> mal_coremelt_idx1;
        bit<32> mal_coremelt_idx2;

        hash(mal_coremelt_idx0, HashAlgorithm.crc16, (bit<32>)0,
        {NUM_RAND1, meta.sip, meta.dip, meta.sport, meta.dport, NUM_RAND2}, (bit<16>)MAL_COREMELT_SIZE);
        hash(mal_coremelt_idx1, HashAlgorithm.crc16, (bit<32>)0,
        {NUM_RAND1, NUM_RAND2, meta.sip, meta.dip, meta.sport, meta.dport}, (bit<16>)MAL_COREMELT_SIZE);
        hash(mal_coremelt_idx2, HashAlgorithm.crc16, (bit<32>)0,
        {meta.sip, meta.dip, meta.sport, meta.dport, NUM_RAND1, NUM_RAND2}, (bit<16>)MAL_COREMELT_SIZE);

        if (hdr.ipv4.protocol == TCP_PROTOCOL){
            mal_coremelt.write(mal_coremelt_idx0, 1);
            mal_coremelt.write(mal_coremelt_idx1, 1);
            mal_coremelt.write(mal_coremelt_idx2, 1);
        }
    }
}
control filter_victimLks_size_classification21_1(inout headers hdr,
                  inout metadata meta,
                  inout standard_metadata_t standard_metadata,
                  in register<bit<32>> victimLks_size) {
    ...
    apply{
        ...
        if (meta.victimLks_size > 1) {
            ...
            meta.classification2_filter1 = 0; //no high load, clean this flag
        }
    }
}

control filter_link_in_victimLks_classification21(inout headers hdr,
                  inout metadata meta,
                  inout standard_metadata_t standard_metadata,
                  in register<bit<32>> victimLks) {
...
control reduce_pktlen_to_flowsz_c_window(inout headers hdr,
                  inout metadata meta,
                  inout standard_metadata_t standard_metadata,
                  in register<bit<32>> reduce_pktlen_to_flowsz_c_cur_wind,
                  in register<bit<32>> reduce_pktlen_to_flowsz_c_cur_wind_pass,
                  in register<bit<32>> reduce_pktlen_to_flowsz_c_full_wind,
                  in register<bit<32>> reduce_pktlen_to_flowsz_c_clean_wind,
                  in register<bit<32>> reduce_pktlen_to_flowsz_c_clean_offset,
                  in register<bit<48>> c_wind_last_ts_p2) {

    apply{
        bit<32> reduce_pktlen_to_flowsz_c_cur_wind_val;
        reduce_pktlen_to_flowsz_c_cur_wind.read(reduce_pktlen_to_flowsz_c_cur_wind_val, 0);
        bit<32> reduce_pktlen_to_flowsz_c_cur_wind_pass_val;
        reduce_pktlen_to_flowsz_c_cur_wind_pass.read(reduce_pktlen_to_flowsz_c_cur_wind_pass_val, 0);

        if (meta.reduce_pktlen_to_flowsz_c_wind_updated == 0 && meta.c_wind_interval_p2 > 100 * 1000){
            meta.reduce_pktlen_to_flowsz_c_wind_updated = 1;
            c_wind_last_ts_p2.write(0, meta.this_ts_val);
            reduce_pktlen_to_flowsz_c_cur_wind_pass.write(0, reduce_pktlen_to_flowsz_c_cur_wind_pass_val + 1);
            reduce_pktlen_to_flowsz_c_clean_offset.write(0, 0);
            if (reduce_pktlen_to_flowsz_c_cur_wind_val == 0){
                reduce_pktlen_to_flowsz_c_cur_wind.write(0, 1);
                reduce_pktlen_to_flowsz_c_full_wind.write(0, 0);
                reduce_pktlen_to_flowsz_c_clean_wind.write(0, 2);
                reduce_pktlen_to_flowsz_c_cur_wind_val = 1;
            } else if (reduce_pktlen_to_flowsz_c_cur_wind_val == 1){
                reduce_pktlen_to_flowsz_c_cur_wind.write(0, 2);
                reduce_pktlen_to_flowsz_c_full_wind.write(0, 1);
                reduce_pktlen_to_flowsz_c_clean_wind.write(0, 0);
                reduce_pktlen_to_flowsz_c_cur_wind_val = 2;
            } else {
                reduce_pktlen_to_flowsz_c_cur_wind.write(0, 0);
                reduce_pktlen_to_flowsz_c_full_wind.write(0, 2);
                reduce_pktlen_to_flowsz_c_clean_wind.write(0, 1);
                reduce_pktlen_to_flowsz_c_cur_wind_val = 0;
            }
        }
    }
}

control reduce_pktlen_to_flowsz_c_update(inout headers hdr,
                  inout metadata meta,
                  inout standard_metadata_t standard_metadata,
                  in register<bit<32>> reduce_pktlen_to_flowsz_c_cur_wind,
                  in register<bit<32>> reduce_pktlen_to_flowsz_c_s0_w0,
                  in register<bit<32>> reduce_pktlen_to_flowsz_c_s1_w0,
                  in register<bit<32>> reduce_pktlen_to_flowsz_c_s2_w0,
                  in register<bit<32>> reduce_pktlen_to_flowsz_c_s0_w1,
                  in register<bit<32>> reduce_pktlen_to_flowsz_c_s1_w1,
                  in register<bit<32>> reduce_pktlen_to_flowsz_c_s2_w1,
                  in register<bit<32>> reduce_pktlen_to_flowsz_c_s0_w2,
                  in register<bit<32>> reduce_pktlen_to_flowsz_c_s1_w2,
                  in register<bit<32>> reduce_pktlen_to_flowsz_c_s2_w2) {
    ...
    apply{
        ...
        bit<32> reduce_pktlen_to_flowsz_c_idx0;
        bit<32> reduce_pktlen_to_flowsz_c_idx1;
        bit<32> reduce_pktlen_to_flowsz_c_idx2;

        hash(reduce_pktlen_to_flowsz_c_idx0, HashAlgorithm.crc16, (bit<32>)0,
        {NUM_RAND1, meta.sip, meta.dip, meta.sport, meta.dport, NUM_RAND2}, (bit<16>)REDUCE_PKTLEN_TO_FLOWSZ_C_SIZE);
        hash(reduce_pktlen_to_flowsz_c_idx1, HashAlgorithm.crc16, (bit<32>)0,
        {NUM_RAND1, NUM_RAND2, meta.sip, meta.dip, meta.sport, meta.dport}, (bit<16>)REDUCE_PKTLEN_TO_FLOWSZ_C_SIZE);
        hash(reduce_pktlen_to_flowsz_c_idx2, HashAlgorithm.crc16, (bit<32>)0,
        {meta.sip, meta.dip, meta.sport, meta.dport, NUM_RAND1, NUM_RAND2}, (bit<16>)REDUCE_PKTLEN_TO_FLOWSZ_C_SIZE);


        bit<32> reduce_pktlen_to_flowsz_c_val0;
        bit<32> reduce_pktlen_to_flowsz_c_val1;
        bit<32> reduce_pktlen_to_flowsz_c_val2;
        REG_WIND_READ(reduce_pktlen_to_flowsz_c_s0, reduce_pktlen_to_flowsz_c_cur_wind_val, reduce_pktlen_to_flowsz_c_val0, reduce_pktlen_to_flowsz_c_idx0)
        REG_WIND_READ(reduce_pktlen_to_flowsz_c_s1, reduce_pktlen_to_flowsz_c_cur_wind_val, reduce_pktlen_to_flowsz_c_val1, reduce_pktlen_to_flowsz_c_idx1)
        REG_WIND_READ(reduce_pktlen_to_flowsz_c_s2, reduce_pktlen_to_flowsz_c_cur_wind_val, reduce_pktlen_to_flowsz_c_val2, reduce_pktlen_to_flowsz_c_idx2)


        bit<32> reduce_pktlen_to_flowsz_c_cur = (reduce_pktlen_to_flowsz_c_val0 < reduce_pktlen_to_flowsz_c_val1) ? reduce_pktlen_to_flowsz_c_val0 : reduce_pktlen_to_flowsz_c_val1;
        reduce_pktlen_to_flowsz_c_cur = (reduce_pktlen_to_flowsz_c_cur < reduce_pktlen_to_flowsz_c_val2) ? reduce_pktlen_to_flowsz_c_cur : reduce_pktlen_to_flowsz_c_val2;


        //update the reduce value to sketches
        reduce_pktlen_to_flowsz_c_cur = reduce_pktlen_to_flowsz_c_cur + standard_metadata.packet_length;

        REG_WIND_WRITE(reduce_pktlen_to_flowsz_c_s0, reduce_pktlen_to_flowsz_c_cur_wind_val, reduce_pktlen_to_flowsz_c_idx0, reduce_pktlen_to_flowsz_c_cur)
        REG_WIND_WRITE(reduce_pktlen_to_flowsz_c_s1, reduce_pktlen_to_flowsz_c_cur_wind_val, reduce_pktlen_to_flowsz_c_idx1, reduce_pktlen_to_flowsz_c_cur)
        REG_WIND_WRITE(reduce_pktlen_to_flowsz_c_s2, reduce_pktlen_to_flowsz_c_cur_wind_val, reduce_pktlen_to_flowsz_c_idx2, reduce_pktlen_to_flowsz_c_cur)
    }
}

control reduce_pktlen_to_flowsz_c_clean(inout headers hdr,
                  inout metadata meta,
                  inout standard_metadata_t standard_metadata,
                  in register<bit<32>> reduce_pktlen_to_flowsz_c_cur_wind_pass,
                  in register<bit<32>> reduce_pktlen_to_flowsz_c_clean_wind,
    ...
    apply{
        ...
        if (reduce_pktlen_to_flowsz_c_cur_wind_pass_val >= 2){
            bit<32> reduce_pktlen_to_flowsz_c_clean_offset_val;
            reduce_pktlen_to_flowsz_c_clean_offset.read(reduce_pktlen_to_flowsz_c_clean_offset_val, 0);
            bit<32> reduce_pktlen_to_flowsz_c_clean_wind_val;
            reduce_pktlen_to_flowsz_c_clean_wind.read(reduce_pktlen_to_flowsz_c_clean_wind_val, 0);

            #ifdef  CLEAN_LARGE
                REG_WIND_CLEAN_512(reduce_pktlen_to_flowsz_c_s0, reduce_pktlen_to_flowsz_c_clean_wind_val, reduce_pktlen_to_flowsz_c_clean_offset_val)
                REG_WIND_CLEAN_512(reduce_pktlen_to_flowsz_c_s1, reduce_pktlen_to_flowsz_c_clean_wind_val, reduce_pktlen_to_flowsz_c_clean_offset_val)
                REG_WIND_CLEAN_512(reduce_pktlen_to_flowsz_c_s2, reduce_pktlen_to_flowsz_c_clean_wind_val, reduce_pktlen_to_flowsz_c_clean_offset_val)

                if (reduce_pktlen_to_flowsz_c_clean_offset_val + 512 > REDUCE_PKTLEN_TO_FLOWSZ_C_SIZE){
                    reduce_pktlen_to_flowsz_c_clean_offset_val = 0;
                } else {
                    reduce_pktlen_to_flowsz_c_clean_offset_val = reduce_pktlen_to_flowsz_c_clean_offset_val + 512;
                }
            #else
                REG_WIND_CLEAN(reduce_pktlen_to_flowsz_c_s0, reduce_pktlen_to_flowsz_c_clean_wind_val, reduce_pktlen_to_flowsz_c_clean_offset_val)
                REG_WIND_CLEAN(reduce_pktlen_to_flowsz_c_s1, reduce_pktlen_to_flowsz_c_clean_wind_val, reduce_pktlen_to_flowsz_c_clean_offset_val)
                REG_WIND_CLEAN(reduce_pktlen_to_flowsz_c_s2, reduce_pktlen_to_flowsz_c_clean_wind_val, reduce_pktlen_to_flowsz_c_clean_offset_val)

                if (reduce_pktlen_to_flowsz_c_clean_offset_val + 128 > REDUCE_PKTLEN_TO_FLOWSZ_C_SIZE){
                    reduce_pktlen_to_flowsz_c_clean_offset_val = 0;
                } else {
                    reduce_pktlen_to_flowsz_c_clean_offset_val = reduce_pktlen_to_flowsz_c_clean_offset_val + 128;
                }
            #endif

            reduce_pktlen_to_flowsz_c_clean_offset.write(0, reduce_pktlen_to_flowsz_c_clean_offset_val);
        }
    }
}

control reduce_pktlen_to_flowsz_c_read(inout headers hdr,
                  inout metadata meta,
                  inout standard_metadata_t standard_metadata,
                  in register<bit<32>> reduce_pktlen_to_flowsz_c_full_wind,
                  in register<bit<32>> reduce_pktlen_to_flowsz_c_s0_w0,
    ...
    apply{
        bit<32> reduce_pktlen_to_flowsz_c_full_wind_val;
        reduce_pktlen_to_flowsz_c_full_wind.read(reduce_pktlen_to_flowsz_c_full_wind_val, 0);
        ...
        REG_WIND_READ(reduce_pktlen_to_flowsz_c_s0, reduce_pktlen_to_flowsz_c_full_wind_val, reduce_pktlen_to_flowsz_c_val0, reduce_pktlen_to_flowsz_c_idx0)
        REG_WIND_READ(reduce_pktlen_to_flowsz_c_s1, reduce_pktlen_to_flowsz_c_full_wind_val, reduce_pktlen_to_flowsz_c_val1, reduce_pktlen_to_flowsz_c_idx1)
        REG_WIND_READ(reduce_pktlen_to_flowsz_c_s2, reduce_pktlen_to_flowsz_c_full_wind_val, reduce_pktlen_to_flowsz_c_val2, reduce_pktlen_to_flowsz_c_idx2)

        meta.reduce_pktlen_to_flowsz_c_full = (reduce_pktlen_to_flowsz_c_val0 < reduce_pktlen_to_flowsz_c_val1) ? reduce_pktlen_to_flowsz_c_val0 : reduce_pktlen_to_flowsz_c_val1;
        meta.reduce_pktlen_to_flowsz_c_full = (meta.reduce_pktlen_to_flowsz_c_full < reduce_pktlen_to_flowsz_c_val2) ? meta.reduce_pktlen_to_flowsz_c_full : reduce_pktlen_to_flowsz_c_val2;
    }
}
control filter_flowsz_c_classification21_4(inout headers hdr,
                  inout metadata meta,
                  inout standard_metadata_t standard_metadata,
                  in register<bit<32>> reduce_pktlen_to_flowsz_c_cur_wind_pass) {
    ...
    apply{
        ...
        if (meta.reduce_pktlen_to_flowsz_c_full < 100 && reduce_pktlen_to_flowsz_c_cur_wind_pass_val >= 1) {
            meta.classification2_filter1 = 1;
        } else {
            meta.classification2_filter1 = 0; //no high load, clean this flag
        }
...
control distinct_sip_dip_sport_dport_window(inout headers hdr,
    ...
    apply{
        ...
        if (meta.distinct_sip_dip_sport_dport_wind_updated == 0 && meta.c_wind_interval_p2 > 100 * 1000){
...
control reduce_id_to_cnt_window(inout headers hdr,
    ...
    apply{
        ...
        if (meta.reduce_id_to_cnt_wind_updated == 0 && meta.c_wind_interval_p2 > 100 * 1000){
...
control filter_cnt_classification21_8(inout headers hdr,
                  inout metadata meta,
                  inout standard_metadata_t standard_metadata,
                  in register<bit<32>> reduce_id_to_cnt_cur_wind_pass) {
...
control filter_cnt_return(inout headers hdr,
                  inout metadata meta,
                  inout standard_metadata_t standard_metadata,
                  in register<bit<32>> mal_crossfire) {

    apply{

        bit<32> mal_crossfire_idx0;
        bit<32> mal_crossfire_idx1;
        bit<32> mal_crossfire_idx2;

        hash(mal_crossfire_idx0, HashAlgorithm.crc16, (bit<32>)0,
        {NUM_RAND1, meta.sip, meta.dip, NUM_RAND2}, (bit<16>)MAL_CROSSFIRE_SIZE);
        hash(mal_crossfire_idx1, HashAlgorithm.crc16, (bit<32>)0,
        {NUM_RAND1, NUM_RAND2, meta.sip, meta.dip}, (bit<16>)MAL_CROSSFIRE_SIZE);
        hash(mal_crossfire_idx2, HashAlgorithm.crc16, (bit<32>)0,
        {meta.sip, meta.dip, NUM_RAND1, NUM_RAND2}, (bit<16>)MAL_CROSSFIRE_SIZE);

        if (hdr.ipv4.protocol == TCP_PROTOCOL){
            mal_crossfire.write(mal_crossfire_idx0, 1);
            mal_crossfire.write(mal_crossfire_idx1, 1);
            mal_crossfire.write(mal_crossfire_idx2, 1);
        }
    }
}
...
control filter_sip_dip_sport_dport_in_mal_coremelt_mitigation1(inout headers hdr,
                  inout metadata meta,
                  inout standard_metadata_t standard_metadata,
                  in register<bit<32>> mal_coremelt) {
    ...
    apply{
        ...
        bit<32> mal_coremelt_val0;
        bit<32> mal_coremelt_val1;
        bit<32> mal_coremelt_val2;

        mal_coremelt.read(mal_coremelt_val0, mal_coremelt_idx0);
        mal_coremelt.read(mal_coremelt_val1, mal_coremelt_idx1);
        mal_coremelt.read(mal_coremelt_val2, mal_coremelt_idx2);

        if (mal_coremelt_val0 == 1 && mal_coremelt_val1 == 1 && mal_coremelt_val2 == 1) {
            meta.mitigation_filter1 = 1;
        } else {
...
control filter_sip_dip_in_mal_crossfire_mitigation2(inout headers hdr,
                  inout metadata meta,
                  inout standard_metadata_t standard_metadata,
                  in register<bit<32>> mal_crossfire) {
    ...
    apply{
        ...
        bit<32> mal_crossfire_val0;
        bit<32> mal_crossfire_val1;
        bit<32> mal_crossfire_val2;

        mal_crossfire.read(mal_crossfire_val0, mal_crossfire_idx0);
        mal_crossfire.read(mal_crossfire_val1, mal_crossfire_idx1);
        mal_crossfire.read(mal_crossfire_val2, mal_crossfire_idx2);

        if (mal_crossfire_val0 == 1 && mal_crossfire_val1 == 1 && mal_crossfire_val2 == 1) {
...
control ctrl_sync(inout headers hdr,
    ...
    apply{
        ...
        DEF_VAR_64(mal_coremelt_val)
        READ_REG_64(mal_coremelt, mal_coremelt_val)
        DEF_VAR_64(mal_crossfire_val)
        READ_REG_64(mal_crossfire, mal_crossfire_val)
        ...
        if (meta.swid == 3) { // The root
            if (hdr.sync.srcSWUID == 3) { // downstream packets
                ...
                SET_HDR_64_OR(hdr.sync.mal_coremelt, mal_coremelt_val)
                SET_HDR_64_OR(hdr.sync.mal_crossfire, mal_crossfire_val)
            }
            // upstream packets
            else {
                ...
                UPDATE_REG_BF_64(mal_coremelt, mal_coremelt_val, hdr.sync.mal_coremelt)
                UPDATE_REG_BF_64(mal_crossfire, mal_crossfire_val, hdr.sync.mal_crossfire)
            }
        }
        // not root
        else {
